...
std::list<value_type<T>> dijkstrasAlgorithm(const WeightedGraph<T> &graph, vertex_type<T> initial_node, vertex_type<T> destination_node)
{
    ...
    while (!q.empty())
    {
        ...
        if (distances[current_node] == infinity<T>())
        {
            break;
        }
        ...
        for (const auto &neighbor : graph.at(current_node))
        {
            ...
            if (s.find(next_node) != s.end())
            {
                continue;
            }

            auto weight = neighbor.second;
            bool relaxed = relax<T>(current_node, next_node, weight, distances, predecessors);
    ...
    std::list<value_type<T>> shortest_path;
    ...
    while (predecessors[current].has_value())
    {
        ...
        current = predecessors.at(current).value();
    }

    if (shortest_path.empty() || initial_node != destination_node)
    {
        shortest_path.push_front(initial_node);
    }
